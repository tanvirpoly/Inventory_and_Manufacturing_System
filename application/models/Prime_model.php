...
public function get_manu_dept_data($id)
  {
  $query = $this->db->select('*')
                  ->from('mdepartment')
                  ->where('md_id',$id)
...
public function get_stock_product()
  ...
  if($_SESSION['role'] <= 2)
    ...
    return $this->db->select('stock.*,products.productName,products.productcode,products.pprice')
              ...
              ->join('products','products.productID = stock.product','left')
              ->where_in('product',$empid)
              ->where_in('stock.compid',$_SESSION['compid'])
              ->get()
              ->result();
    }
}
...
//   if($_SESSION['role'] <= 2)
//     {
    $query = $this->db->select("products.*,categories.categoryName,sma_units.unitName,mdepartment.mdName")
                      ->from('products')
                      ->join('categories','categories.categoryID = products.categoryID','left')
                      ->join('sma_units','sma_units.id = products.unit','left')
                      ->join('mdepartment','mdepartment.md_id = products.mdid','left')
...
//   else
//     {
//     $query = $this->db->select("delivery_product.pid,delivery.empid,products.*,categories.categoryName,sma_units.unitName,mdepartment.mdName")
//                       ->from('delivery_product')
...
public function get_sale_batch_data($id)
  {
  $query = $this->db->select('*')
                ->from('stock')
                ->where('product',$id)
...
public function get_product_batch_stock($pid,$batch,$compid)
  {
  $query = $this->db->select('*')
                ->from('stock')
                ->where('product',$pid)
